...
func TestConfigReader(t *testing.T) {
	...
	t.Run("Read incorrect properties", func(t *testing.T) {
		...
		require.Error(t, err)
		require.Equal(t, "org item 1 in configuration doesn't contain required field name\norg item 1 in configuration doesn't contain required field initialAdminLoginOrEmail", err.Error())
	})
	...
	t.Run("Can read correct properties", func(t *testing.T) {
		...
		require.Equal(t, "user2@example.org", cfg[0].CreateOrgs[1].InitialAdminLoginOrEmail)
